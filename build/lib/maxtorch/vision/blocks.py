...
class ResidualBlock(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, stride=1, downsample=None,group_norm=False):
        super().__init__()
...
class ASPPBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        gp = nn.functional.interpolate(gp, size=x.shape[2:], mode="bilinear", align_corners=True)
        res.append(gp)
...
class PANetBlock(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels):
        ...
        self.bottom_up_convs = nn.ModuleList([
            nn.Conv2d(out_channels, out_channels, 3, stride=2, padding=1) for _ in range(len(in_channels_list) - 1)
        ])
    ...
    def forward(self, inputs):
        ...
        for i in range(len(laterals) - 1, 0, -1):
            upsample = nn.functional.interpolate(laterals[i], size=laterals[i-1].shape[2:], mode="nearest")
...
class BiFPNLayer(nn.Module):
    def __init__(self, num_levels, out_channels):
        ...
        self.eps = 1e-4
        self.convs = nn.ModuleList([
            nn.Conv2d(out_channels, out_channels, 3, padding=1, groups=out_channels, bias=False) for _ in range(num_levels)
        ])
        self.relu = nn.ReLU(inplace=True)
    def forward(self, feats):
        ...
        for i in range(self.num_levels - 2, -1, -1):
            ...
            w = w / (w.sum() + self.eps)
            up = nn.functional.interpolate(td[0], size=feats[i].shape[2:], mode="nearest")
...
class SwinTransformerBlock(nn.Module):
    ...
    """
    def __init__(self, dim, num_heads, window_size, shift_size=0, mlp_ratio=4.0, dropout=0.0, attn_dropout=0.0, drop_path=0.0):
        super().__init__()
        self.dim = dim
