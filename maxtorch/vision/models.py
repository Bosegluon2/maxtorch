    ...
    def __init__(self, in_channels=1, out_channels=1, features=[64, 128, 256, 512]):
        ...
        for i in range(self.depth):
            for j in range(1, self.depth - i):
                self.conv_blocks[f"conv{i}_{j}"] = self._conv_block(features[i] * (j + 1), features[i])
        # 上采样部分
        for feature in reversed(features):
    ...
    def forward(self, x):
        ...
        for j in range(1, self.depth):
            for i in range(self.depth - j):
                upsampled = nn.functional.interpolate(xs[i+1][j-1], size=xs[i][0].shape[2:], mode="bilinear", align_corners=True)
...
class DeepLabV3PlusBlock(nn.Module):
    ...
    """
    def __init__(self, in_channels, num_classes, aspp_out_channels=256, atrous_rates=(12, 24, 36)):
        super().__init__()
